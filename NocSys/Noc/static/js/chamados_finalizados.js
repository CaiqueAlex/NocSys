...
async function loadChamadosFinalizados() {
  ...
  content.innerHTML = `
    <h2 style="display: flex; align-items: center; justify-content: space-between;">
      ...
      <button id="btnRecarregarChamadosFinalizados"
  ...
  try {
    ...
    if (chamados.length === 0) {
      content.innerHTML = '<h2>Chamados Finalizados</h2><p>Nenhum chamado finalizado encontrado.</p>';
      return;
    }
    ...
    <h2 style="display:flex;align-items:center;justify-content:space-between;">
      <span>Chamados Finalizados</span>
      <button id="btnRecarregarChamadosFinalizados" title="Recarregar" style="background:none;border:none;color:#38bdf8;font-size:22px;cursor:pointer;">🔄</button>
    </h2>
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <div>
              ...
              <div style="font-size:0.88em;color:#d1fae5;opacity:.84;margin-top:2px;">
                ...
                <span style="color:#fbbf24;"> | Encerrado: ${niceDate(encerrado)}</span>
              </div>
            </div>
            <button
              onclick="toggleFinalizadoDetalhes(this)"
              style="background: #2563eb; color: #fff; border:none; font-weight:600; border-radius:5px; font-size:15px; padding:7px 16px;transition:background 0.25s; box-shadow:0 1px 6px #15213e;cursor:pointer">
                Mostrar
            </button>
          </div>
...
async function toggleFinalizadoDetalhes(btn) {
  ...
  try {
    ...
    convo += `
      <div style="margin-top:13px; padding:18px 12px 12px 16px; border-radius:8px; background:#22283b; color:#f8fafc; box-shadow:0 2px 12px #10101040;">
        ...
        <div style="margin-top:12px;font-size:.98em;color:#FBBF24;">Chamado finalizado - somente leitura</div>
      </div>
    `;
...
document.addEventListener('DOMContentLoaded', loadChamadosFinalizados);